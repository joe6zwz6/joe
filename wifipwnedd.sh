...
hackingwifi() {
	...
	echo -e "${yellowColour}[1]$grayColour Handshake Attack"
	echo -e "${yellowColour}[2]$grayColour PMKID Attack"
	echo -e "${yellowColour}[3]$grayColour DoS Attack"
...
crackingpass() {
	...
	echo -e "${yellowColour}[1]$grayColour Force Brute .cap"
...
if [ $(id -u) -ne 0 ]; then
	...
	echo -e "${greenColour}[+]${grayColour} Github: https://github.com/joe6zwz6"
	echo -e "${greenColour}[+]${grayColour} Discord ID: zspe"
	echo -e "${greenColour}[+]${grayColour} Instagram: 6zwz6"
	echo -ne "\n${greenColour}[+]$grayColour Enter to continue" && read 
	updatepackages
	monitormode
	while true; do
